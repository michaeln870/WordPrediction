...
alltxt <- c(read_lines("../Data/en_US/en_US.blogs.txt"),
            read_lines("../Data/en_US/en_US.news.txt"),
            read_lines("../Data/en_US/en_US.twitter.txt")) %>%
...
files <- data.frame(File = c("en_US.blogs.txt","en_US.news.txt","en_US.twitter.txt",""),
                    ...
                    Lines = c("899,288","1,010,242","2,360,148","4,269,678"),
                    Words = c("37,546,239","34,762,395","30,093,372","102,402,006")
                    )
knitr::kable(files)
```
...
trigrams_20 <- trigrams[1:20,] %>%
  mutate(ngrams = paste(word1_2, word3)) %>%
  select(ngrams,n)
fourgrams_20 <- fourgrams[1:20,] %>%
  mutate(ngrams = paste(word1_3, word4)) %>%
  select(ngrams,n)
fivegrams_20 <- fivegrams[1:20,] %>%
  mutate(ngrams = paste(word1_4, word5)) %>%
  select(ngrams,n)
...
```{r trigrams network, echo=FALSE}
#Graphing network trigrams
set.seed(1111)
a <- grid::arrow(type = "closed", length = unit(.15, "inches"))

ngram_graph <- trigrams %>%
  graph_from_data_frame() %>%
  ggraph(layout = "fr") +
  geom_edge_link(aes(edge_alpha = n), show.legend = FALSE,
                 arrow = a, end_cap = circle(.07, 'inches')) +
...
```{r fourgrams network, echo=FALSE}
#Graphin network fourgrams
set.seed(1111)
a <- grid::arrow(type = "closed", length = unit(.15, "inches"))

ngram_graph <- fourgrams %>%
  graph_from_data_frame() %>%
  ggraph(layout = "fr") +
  geom_edge_link(aes(edge_alpha = n), show.legend = FALSE,
                 arrow = a, end_cap = circle(.07, 'inches')) +
...
```{r fivegrams network, echo=FALSE}
#Graphing network fivegrams
set.seed(1111)
a <- grid::arrow(type = "closed", length = unit(.15, "inches"))

ngram_graph <- fivegrams %>%
  graph_from_data_frame() %>%
  ggraph(layout = "fr") +
  geom_edge_link(aes(edge_alpha = n), show.legend = FALSE,
                 arrow = a, end_cap = circle(.07, 'inches')) +
